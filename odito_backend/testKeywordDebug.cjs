...
const BASE_URL = 'localhost:5000'; // Backend runs on port 5000
const USE_HTTPS = false; // Set to true if using HTTPS
...
function makeRequest(method, path, params = '') {
    return new Promise((resolve, reject) => {
        ...
        const options = {
            ...
            port: BASE_URL.split(':')[1] || (USE_HTTPS ? 443 : 80),
            path: url,
            method: method,
            headers: {
...
async function testKeywordDetail(projectId, keyword) {
    ...
    try {
        ...
        const hasRealMonthlyData = hasMonthly && response.data.monthly.some(v => v !== (response.data.vol || 0));
        ...
        logTest('Keyword detail has REAL monthly data (not constant)', hasRealMonthlyData);
