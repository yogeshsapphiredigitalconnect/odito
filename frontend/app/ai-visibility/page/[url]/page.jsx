...
export default function AIPageDetailDeepDive(props) {
  ...
  useEffect(() => {
    ...
    const fetchPageData = async () => {
      try {
        ...
        console.log("🔍 FRONTEND RAW API RESPONSE:", pageScoreRes);
        console.log("🔍 FRONTEND RECEIVED DATA:", pageScoreRes.data);
        ...
        console.log("🔍 FRONTEND SCORE BREAKDOWN:");
        console.log("  - finalScore:", pageScoreRes.data.finalScore);
        console.log("  - categoryScores.ai_impact:", pageScoreRes.data.categoryScores?.ai_impact);
        console.log("  - categoryScores:", pageScoreRes.data.categoryScores);
  ...
  const getRuleName = (ruleId) => {
    ...
    return ruleNames[ruleId] || 'Unknown Rule';
  };
  ...
  console.log("🔍 RENDER TIME SCORE BREAKDOWN:");
  console.log("  - categoryScores:", categoryScores);
  console.log("  - categoryScores.ai_impact:", categoryScores.ai_impact);
  console.log("  - finalScore:", pageScore.finalScore);
  console.log("🔍 SCORE STRIP MAPPING:");
  console.log("  - AI Impact using finalScore:", pageScore.finalScore);
  console.log("  - Should be using categoryScores.ai_impact:", categoryScores.ai_impact);
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <details className="group bg-surface border border-border rounded-lg">
              <summary className="flex items-center justify-between p-4 cursor-pointer select-none">
                <div className="flex items-center gap-3">
                  <div className="size-8 rounded flex items-center justify-center bg-surface-highlight text-purple-400">
            ...
            <details className="group bg-surface border border-border rounded-lg">
              <summary className="flex items-center justify-between p-4 cursor-pointer select-none">
                <div className="flex items-center gap-3">
                  <div className="size-8 rounded flex items-center justify-center bg-surface-highlight text-amber-400">
        ...
        <div className="border-b border-border mb-4">
          <div className="flex gap-8 py-3">
            {[
              ...
              const tabIssueCount = issues.filter(issue => issue.category === tab.value).length;
              return (
                <button
                  key={tab.value}
          ...
          <div className="overflow-x-auto rounded-lg border border-border">
            {(() => {
              const filteredIssues = issues.filter(
                issue => issue.category === activeCategory
              );
              ...
              <tbody className="divide-y divide-border bg-surface">
                {pageScore?.ruleBreakdown && pageScore.ruleBreakdown.length > 0 ? (
                  pageScore.ruleBreakdown
                    .filter(rule => rule.category === activeCategory)
                    .map((rule, index) => {
                      const ruleScore = rule.score || 0;
                      const status = ruleScore >= 70 ? 'Passed' : ruleScore >= 40 ? 'Warning' : 'Failed';
                      const statusColor = ruleScore >= 70 ? 'text-success' : ruleScore >= 40 ? 'text-warning' : 'text-danger';
                      ...
                      return (
                        <tr key={index} className="hover:bg-surface-highlight/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-white">{getRuleName(rule.rule_id)}</td>
