#!/usr/bin/env python3
"""
Test page analysis with debug output - fixed version
"""
...
def test_headless_debug():
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            # Look for accessibility-specific issues 
            accessibility_issues = []
            for issue in issues:
                rule_id = issue.get('rule_id')
                if isinstance(rule_id, int) and 213 <= rule_id <= 236:
                    accessibility_issues.append(issue)
                elif isinstance(rule_id, str):
                    # Check if rule code contains accessibility keywords
                    rule_code = issue.get('rule_code', '')
                    if any(keyword in rule_code for keyword in ['AXE_', 'ARIA_', 'KEYBOARD_', 'FOCUS_', 'DOM_', 'BUTTONS_', 'LINKS_', 'SMALL_', 'HEADING_', 'HTML_', 'UNREACHABLE_', 'SKIP_']):
                        accessibility_issues.append(issue)
            
            print(f"\n🔍 ACCESSIBILITY RULES ANALYSIS:")
            ...
            if accessibility_issues:
                ...
                # Let's check what headless data actually made it to the rules
                print(f"\n🔍 CHECKING HEADLESS DATA IN RULES:")
                # We need to check what the rules actually received
